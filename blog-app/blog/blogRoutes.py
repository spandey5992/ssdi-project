...
@login_required
def home():
    top5posts = db.session.query(Post).filter(Post.userid==current_user.id).order_by(desc(Post.created_date)).limit(5).all()
    ...
    top5comments = db.session.query(Comment).filter(Comment.userid==current_user.id).order_by(desc(Comment.created_date)).limit(5).all()
...
def create_post():
    if request.method == 'POST':
        ...
        new_post = Post(userid=post_userid, title=title, content=content, tags=tag)
...
def post(id):
    ...
    if current_user.is_authenticated:
        presence = db.session.query(Like).filter(Post.id==id, User.id==current_user.id).first()
        if presence:
            liked = 1
    post.liked = liked
    text = post.content
    ...
    ids =[]
    for word in text:
        searched_post = db.session.query(Post).filter(Post.title.like('%'+word+'%')).all()
    ...
    searched_post = db.session.query(Post).filter(Post.id.in_(postid_count)).limit(5).all()
...
def editpost(id):
    if request.method == 'POST':
        ...
        post_userid = current_user.id
        tag = request.form.get("tag")
        db.session.query(Post).filter(Post.id==id).update({'title':title, 'content':content, 'tags':tag})
        db.session.commit()
        flash('Post Edited!')
        return redirect(url_for('blogRoutes.post', id=id))
...
def search():
    ...
    postid_count.sort(key = lambda x: -x[1])
    postid_count = [x[0] for x in postid_count]
    searched_post = db.session.query(Post).filter(Post.id.in_(postid_count)).all()   
